...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      ...
      // Also search in profile preferences (email, customerCode)
      where.OR.push({
        profile: {
          preferences: {
            path: ["email"],
    ...
    try {
      ...
      customers = customers.filter((c: any) => {
        const isPlaceholder =
          c.phone?.startsWith("GROUP_") ||
          (c.profile?.preferences as any)?.isGroupPlaceholder === true;
        return !isPlaceholder;
      });

      // Apply advanced filters in memory (for JSON fields)
      if (membershipStatus) {
        const statuses = membershipStatus.split(",");
        customers = customers.filter((c: any) => {
          const rank = (c.profile?.preferences as any)?.rank;
          return rank && statuses.includes(rank);
        });
      }
      ...
      if (source) {
        ...
        customers = customers.filter((c: any) => {
          const customerGroup = (c.profile?.preferences as any)?.customerGroup;
          return customerGroup && sources.includes(customerGroup);
        });
      }

      if (customerGroup) {
        const groups = customerGroup.split(",");
        customers = customers.filter((c: any) => {
      ...
      if (
        ...
        if (search) {
          filteredMock = mockCustomers.filter(
            (c) =>
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, phone, dateOfBirth, gender, notes } = body;
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "Database connection failed, returning mock response:",
          error.message,
        );
        return successResponse(
          {
