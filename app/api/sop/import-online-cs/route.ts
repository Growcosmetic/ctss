// ============================================
// SOP - Import Online CS SOPs (Pre-configured)
// ============================================
...
// Read online CS SOPs from JSON file
const onlineCSOPS = JSON.parse(
  readFileSync(
    join(process.cwd(), "core", "data", "onlineCSOPS.json"),
    "utf-8"
  )
);
...
export async function POST(req: Request) {
  try {
    ...
    console.error("Import online CS SOPs error:", err);
    return NextResponse.json(
      {
        success: false,
        error: err.message || "Failed to import online CS SOPs",
      },
      { status: 500 }
    );
  }
