// ============================================
// SOP - Import Stylist SOPs (Pre-configured)
// ============================================
...
// Read stylist SOPs from JSON file
const stylistSOPs = JSON.parse(
  readFileSync(
    join(process.cwd(), "core", "data", "stylistSOP.json"),
    "utf-8"
  )
);
...
export async function POST(req: Request) {
  try {
    ...
    console.error("Import stylist SOPs error:", err);
    return NextResponse.json(
      {
        success: false,
        error: err.message || "Failed to import stylist SOPs",
      },
      { status: 500 }
    );
  }
